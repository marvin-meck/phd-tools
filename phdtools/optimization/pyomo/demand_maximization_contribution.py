...
def create_abstract_model():
    ...
    model = BaseModel("Demand maximization with constant contribution")

    add_consumer_preference_model(model)

    model.CONTRIBUTION_MARGIN_EURO = pyo.Param()

    @model.Constraint()
    def contribution_margin_constr(block):
        return (
            block.variableCostsEuro + block.CONTRIBUTION_MARGIN_EURO <= block.priceEuro
        )

    @model.Objective(sense=pyo.maximize)
    def obj(block):
        return block.marketShare
